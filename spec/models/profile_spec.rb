...
RSpec.describe Profile, type: :model do
  ...
  describe 'Validations' do
    context 'when firstname is present' do
      it 'is valid' do
        profile.valid?
        expect(profile.errors[:first_name]).to be_blank
      end
    end

    context 'when firstname is absent' do
      it 'is invalid' do
        profile.first_name = ''
        profile.save
    ...
    context 'when lastname is present' do
      it 'is valid' do
        profile.valid?
        expect(profile.errors[:last_name]).to be_blank
      end
    end

    context 'when lastname is absent' do
      it 'is invalid' do
        profile.last_name = ''
        profile.save
